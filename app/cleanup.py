...
# Отдельно от списка ниже: короткий сегмент, начинающийся со слова «субтитры», —
# почти всегда подпись из обучающих субтитров, а вариантов у неё бесконечно много
# («Субтитры субтитров Н.Новикова» просочилось мимо списка на реальном эфире).
# Цена ошибки — потерянная реплика вида «включи субтитры», что в жизни редкость.
SUBTITLE_PREFIX = "субтитры"
SUBTITLE_PREFIX_MAX_WORDS = 6
...
class SegmentFilter:
    ...
    def verdict(self, event):
        ...
        if (reason is None and word_count <= SUBTITLE_PREFIX_MAX_WORDS
                and norm.startswith(SUBTITLE_PREFIX)):
            reason = "marker"
